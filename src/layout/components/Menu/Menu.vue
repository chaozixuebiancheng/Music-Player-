...
const showMySonglist = ref(false) // 控制我的歌单栏显示的变量
// 监听按键按下事件
onMounted(() => {
...
<template>
  <aside class="w-64 dark:border-gray-600 h-full border-r box-border flex flex-col overflow-x-hidden">
    <!-- 个人信息界面 -->
    <div class="flex flex-col items-center p-4 gap-2">
      <span class="relative flex h-24 w-24 shrink-0">
        <img class="aspect-square h-full w-full rounded-full" src="@/assets/gthub-avatar.png" />
      </span>
    ...
    <nav class="flex flex-col p-4 space-y-4 flex-1 h-full box-border overflow-x-hidden">
      <div v-for="(item, index) in MenuData" :key="index" class="w-full flex flex-col gap-1">
        ...
        <router-link
          ...
          :to="item2.router" v-for="(item2, index2) in item.children" :data-index="index + '-' + index2" :key="index2"
          :class="menuStore.menuIndex == index + '-' + index2
      ...
      <!-- 登陆后将会显示我的歌单-->
      <div class="w-full flex flex-col gap-1 " v-if="showMySonglist">
        <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400">
          我的歌单
        </h3>
        <el-scrollbar class="flex-1 overflow-x-hidden">
          <router-link
            ...
            v-for="(item, index) in playlists" :to="`/songlist?id=${item.id}`" :key="index">
            <el-image class="w-7 h-7 rounded-lg" :src="item.coverImgUrl + '?param=50y50'" :alt="item.name" />
