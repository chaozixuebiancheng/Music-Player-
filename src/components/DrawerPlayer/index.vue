...
// 处理歌词字符串，将换行符替换为 HTML <br /> 标签，同时去除前后的空格。
...
<template>
  <el-drawer :style="{ '--track-cover-url': `url(${currentSong.cover})` }" v-model="drawer" :direction="direction"
    style="box-shadow: none" size="100%" :modal="false" :showClose="false" class="drawer-bg">
    <template #header>
          ...
          <div class="items-center justify-center flex flex-col h-full w-full">
            ...
            <div class="flex flex-col items-center w-96 mt-5">
              ...
              <div class="flex gap-2 w-full items-center pt-2">
                ...
                <el-slider v-model="currentTime" :show-tooltip="false" @change="changeCurrentTime" :max="duration"
              ...
              <div>
                <div class="flex items-center">
                  ...
                  <span class="text-sm dark:text-gray-300" v-if="commenTotal !== 0">
        ...
        <div class="flex-[50%] max-w-[50%] md:flex hidden h-full items-center justify-center">
            ...
            <div class="items-center justify-center flex h-full">
              <el-scrollbar class="flex items-center justify-center w-full"
                ...
                <ul :style="scrollStyle">
                  <li v-for="(item, index) in lyricsData.lines" :key="index" :class="[
          ...
          <template v-else>
            <div
              ...
              v-html="parseLyricInfo(lyricsData.remark ?? '')" v-if="SettingStore.isOriginalParsed"></div>
          </template>
        </div>
      </div>
    </div>
